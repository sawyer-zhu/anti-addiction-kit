            ...
            if(user.getAccountType() != AntiAddictionKit.USER_TYPE_ADULT){
                ...
                if(restrictType == 2){
                    ...
                    if(user.getAccountType() <= AntiAddictionKit.USER_TYPE_UNKNOWN){
                        ...
                        if((!isLogin && remainTime <= 3 * 60) || remainTime <= 0 ) {
                            if (TimeUtil.isHoliday()) {
                                int min = AntiAddictionKit.getCommonConfig().getChildHolidayTime();
                                description = AntiAddictionKit.getCommonConfig().strKickOutHoliday;
                            } else {
                    ...
                    title = "健康游戏提示";
                    description = AntiAddictionKit.getCommonConfig().strLoginFailForTimeLimit;
                }
            }
