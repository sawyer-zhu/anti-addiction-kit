...
class IdentifyChargeAmountService extends Service {
    ...
    async updateAmount(identify, amount) {
        ...
        await antiAddictionKit.beginTransactionScope(async conn => {
            ...
            if(result.affectedRows === 0){
                const insert_result = await conn.insert('identify_charge_amounts', {amount, identify, month});
    ...
    async checkPay(identify, amount) {
        const REASON = {
        ...
        if (age < 8) {
            ...
            let total_amount = await antiAddictionKit.get('identify_charge_amounts', {identify, month});
...
module.exports = IdentifyChargeAmountService;